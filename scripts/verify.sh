...
panel_surface_probe() {
  local port_line port ok _attempt
  ...
  PANEL_PROBE_CID="$(docker run -d \
    ...
    -e SESSION_SECRET=verify-session-secret-strong-value-123456 \
    -e RCON_SECRET_KEY=1111111111111111111111111111111111111111111111111111111111111111 \
...
startup_secret_probe() {
  local install_dir argv_file secret_cfg
...
EOF
  ...
  RCON_PASSWORD="probe-rcon-secret-value" \
    CS2_GSLT="probe-gslt-secret-value" \
    CS2_INSTALL_DIR="${install_dir}" \
    CS2_ARGV_FILE="${argv_file}" \
    configs/examples/startup/server-start.sh

  if grep -Fq "probe-rcon-secret-value" "${argv_file}"; then
    printf 'RCON password leaked into startup argv\n' >&2
    exit 1
  fi
  if grep -Fq "probe-gslt-secret-value" "${argv_file}"; then
    printf 'GSLT leaked into startup argv\n' >&2
    exit 1
  fi
  grep -Fq '+exec' "${argv_file}"
  grep -Fq 'cs2-server-ops-secrets.cfg' "${argv_file}"
  grep -Fq 'rcon_password "probe-rcon-secret-value"' "${secret_cfg}"
  grep -Fq 'sv_setsteamaccount "probe-gslt-secret-value"' "${secret_cfg}"
}
